...
class CheckAzureRMVirtualNetworkGatewayConnected < Sensu::Plugin::Check::CLI
  ...
  def run
    ...
    networkClient = usage.buildVirtualNetworkClient(tenant_id, client_id, client_secret, subscription_id)

    primary_result = networkClient.get(resource_group_name, primary_name)
    ...
    secondary_result = networkClient.get(resource_group_name, secondary_name)
